...
export function chartPointsForExercise(entries: EntryRecord[], exercise: string): ChartPoint[] {
  const matching = entries
    ...
    .sort((a, b) => {
      const byDate = a.workout_date.localeCompare(b.workout_date);
  ...
  return matching.map((entry) => {
    ...
    return {
      date: chartDatetime(entry.workout_date, idx).toISOString(),
      total: entry.volume,
    };
  });
}
...
export function chartDataKey(entries: EntryRecord[], exercise: string): string {
  return entries
    ...
    .map(
      (entry) =>
        `${entry.index}:${entry.workout_date}:${entry.logged_at}:${entry.volume}:${entry.set_values.join("|")}`,
    )
    .join(";");
}
